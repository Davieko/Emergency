...
public class Simulator {
	...
	private PriorityQueue<Patient> waitingRoom; //contiene pazienti in attesa di tutti i tipi  
	int freeStudios; //numero studi liberi
...
//		Patient.ColorCode colore = ColorCode.WHITE;
	...
	public void run() {
		while(!this.queue.isEmpty()) {
			//finchè non è vuota estraggo dalla coda
	...
	private void processEvent(Event e) {
		...
		case TRIAGE:
			...
			//se il colore assegnato è bianco allora schedulo l'evento timeout dopo la durata timeout_white
		...
		case FREE_STUDIO:
			if(this.freeStudios == 0)
				return;
			//quale paziente ha il diritto di entrare? Lo faccio uscire con poll
...
//				System.out.println("ERRORE: TIMEOUT CON COLORE "+colore);
	...
	public String toString() {
		return "Simulator [queue=" + queue + ", patients=" + patients + ", freeStudios=" + freeStudios + ", totStudios="
				+ totStudios + ", numPatients=" + numPatients + ", T_ARRIVAL=" + T_ARRIVAL + ", DURATION_TRIAGE="
				+ DURATION_TRIAGE + ", DURATION_WHITE=" + DURATION_WHITE + ", DURATION_YELLOW=" + DURATION_YELLOW
				+ ", DURATION_RED=" + DURATION_RED + ", TIMEOUT_WHITE=" + TIMEOUT_WHITE + ", TIMEOUT_YELLOW="
				+ TIMEOUT_YELLOW + ", TIMEOUT_RED=" + TIMEOUT_RED + ", startTime=" + startTime + ", endTime=" + endTime
				+ ", patientsTreated=" + patientsTreated + ", patientsAbandoned=" + patientsAbandoned
				+ ", patientsDead=" + patientsDead + "]";
	}
}
